...
replaceNAs.df$weekend <- weekdays(as.Date(replaceNAs.df$date)) %in% c("Sunday","Saturday")
g <-ggplot(replaceNAs.df,aes(interval,steps))
g  + facet_grid(. ~ weekend) +stat_summary(fun.data = "mean_cl_boot", colour = "red")
#meanPerInterval.replaceNAs <- sapply(split(replaceNAs.df,list(replaceNAs.df$interval)),function(x) mean(x$steps,na.rm=TRUE) )
